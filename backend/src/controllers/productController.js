...
exports.listBestSellers = async (req, res) => {
    try {
        const bestSellers = await product.findAll({
            ...
            include: [
                {
                    model: product_inventory,
                    as: 'product_inventories',
                    attributes: ['size', 'price', 'quantity'] // Chỉ các cột cần thiết
                },
                {
                    model: category,
                    as: 'category',
            ...
            subQuery: false // Tránh lỗi subquery ảnh hưởng đến GROUP BY
        });

        // Format kết quả
        const formatted = bestSellers.map(p => {
            const inventories = p.product_inventories || [];
            ...
            inventories.forEach(inv => {
                if (inv.price !== null && inv.price !== undefined) {
                    if (minPrice === null || inv.price < minPrice) minPrice = inv.price;
                    if (maxPrice === null || inv.price > maxPrice) maxPrice = inv.price;
                }

                if (inv.size && !sizes.includes(inv.size)) sizes.push(inv.size);
            });
            ...
            return {
                ...
                category: p.category ? {
                    ...
                    name: p.category.display_text
                } : null,
                subCategory: p.subCategory_id ? {
                    id: p.subCategory_id,
                    name: `Subcategory ${p.subCategory_id}` // Nếu cần tên thực, cần join bảng subcategory
                } : null,
                ...
                sizes,
                in_stock: !p.out_of_stock,
                reviews_count: p.reviews_count,
                created_at: p.creation_at,
                updated_at: p.modified_at,
                small_image: p.small_image,
                total_quantity_sold: p.get('total_quantity_sold')
            };
        });
