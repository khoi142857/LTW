...
const createOrderItems = async (orderId, items, orderStatus = 'pending', transaction, req, res, affiliateInfoFromExtraData = null) => {
  try {
    ...
    // Get affiliate info: prefer MoMo extraData if provided
    let affiliateLinks = [];

    if (affiliateInfoFromExtraData) {
      try {
        affiliateLinks = Array.isArray(affiliateInfoFromExtraData)
          ? affiliateInfoFromExtraData
          : [affiliateInfoFromExtraData]; // cho phép 1 hoặc nhiều sản phẩm
      } catch (e) {
        console.error("Invalid affiliateInfo from extraData:", e.message);
      }
    } else {
      affiliateLinks = getAffiliateInfo(req) || [];
    }

    console.log(`Processing ${items.length} order items with ${affiliateLinks.length} affiliate links`);
    ...
    affiliateLinks.forEach(link => {
      ...
      if (
        !mostRecentAffiliatesByProduct[productId] ||
...
const placeOrderMomo = async (req, res) => {
  try {
    ...
    const orderId = `MOMO-${userId}-${uuidv4()}`;
    const requestId = `${partnerCode}-${uuidv4()}`;
...
const verifyMomo = async (req, res) => {
  ...
  try {
    const userId = req.user_id;
    const { orderId, requestId: clientRequestId, address, amount } = req.body;
    ...
    // Tạo requestId mới để query MoMo (khác với clientRequestId)
    const newRequestId = `${orderId}-${Date.now()}`;
    
    // Tạo chữ ký (signature) cho truy vấn trạng thái thanh toán
    const rawSignature = `accessKey=${process.env.MOMO_ACCESS_KEY}&orderId=${orderId}&partnerCode=${process.env.MOMO_PARTNER_CODE}&requestId=${newRequestId}`;
    const signature = createSignature(rawSignature);
    ...
    // Gọi API query trạng thái thanh toán MoMo
    ...
    // Thêm affiliate_info từ extraData (nếu có)
    let affiliateInfo = null;
    if (extraData.affiliate_info) {
      try {
        const parsed = JSON.parse(extraData.affiliate_info);
        // Gắn vào cookie (dự phòng) và truyền cho hàm createOrderItems
        req.cookies = req.cookies || {};
        req.cookies.affiliate_info = encodeURIComponent(extraData.affiliate_info);
        affiliateInfo = parsed; // ✅ Gửi object/array chứ không phải chuỗi
      } catch (e) {
        logger.error(`Error processing affiliate info from MoMo: ${e.message}`);
      }
    }

    // Tạo order_items và xử lý affiliate (nếu có)
    await createOrderItems(newOrder.order_id, items, 'processing', transaction, req, res, affiliateInfo);
